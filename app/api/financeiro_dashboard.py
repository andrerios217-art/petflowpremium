...
def dashboard_financeiro(empresa_id: int, db: Session = Depends(get_db)):
    ...
    # ENTRADAS HOJE
    # =========================
    entradas_hoje = db.query(
        func.coalesce(func.sum(FinanceiroReceber.valor_pago), 0)
    ).filter(
    ...
    saidas_hoje = db.query(
        func.coalesce(func.sum(FinanceiroPagar.valor_pago), 0)
    ).filter(
    ...
    # CONTAS A RECEBER
    # =========================
    pendente_hoje = db.query(
        func.coalesce(func.sum(FinanceiroReceber.valor), 0)
    ).filter(
        FinanceiroReceber.empresa_id == empresa_id,
    ...
    # CONTAS A PAGAR
    # =========================
    pagar_aberto = db.query(
        func.coalesce(func.sum(FinanceiroPagar.valor), 0)
    ).filter(
        FinanceiroPagar.empresa_id == empresa_id,
        FinanceiroPagar.status != "PAGO"
    ).scalar()

    pagar_vencido = db.query(
        func.coalesce(func.sum(FinanceiroPagar.valor), 0)
    ).filter(
        FinanceiroPagar.empresa_id == empresa_id,
